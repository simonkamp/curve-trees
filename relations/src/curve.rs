...
pub fn curve_check<F: Field, Cs: ConstraintSystem<F>>(
    ...
    // x^3 + A*x^2 + B - y^2 = 0
    cs.constrain(
        LinearCombination::<F>::from(x_o) + LinearCombination::<F>::from(x_r).scalar_mul(a) + b - y_r,
    )
}
...
pub fn incomplete_curve_addition<F: Field, Cs: ConstraintSystem<F>>(
    ...
    cs.constrain(LinearCombination::<F>::from(delta_x_r_x_l) - (prms.y_r.clone() - prms.y_l.clone()));

    // delta * (x_o - x_l) = - y_o - y_l
    let (_, _, delta_x_o_x_l) = cs.multiply(delta_lc.clone(), prms.x_o.clone() - prms.x_l.clone());
    cs.constrain(LinearCombination::<F>::from(delta_x_o_x_l) - (-prms.y_o.clone() - prms.y_l.clone()));

    // delta * delta = x_o + x_r + x_l
    let (_, _, delta2) = cs.multiply(delta_lc.clone(), delta_lc.clone());
    cs.constrain(
        prms.x_o.clone() + prms.x_r.clone() + prms.x_l.clone() - LinearCombination::<F>::from(delta2),
    );
}
...
pub fn checked_curve_addition<F: Field, Cs: ConstraintSystem<F>>(
    ...
    not_zero(cs, prms.x_l.clone() - prms.x_r.clone(), x_l_minus_x_r_inv_lc);
...
pub fn re_randomize<F: Field, C: SWModelParameters<BaseField = F>, Cs: ConstraintSystem<F>>(
    ...
    for i in 1..m + 1 {
        ...
        let (index, x_l_minus_x_r_inv, delta, acc_i_x, acc_i_y) = match &r_bits {
            ...
            Some(random_bits) => {
                ...
                blinding_accumulator = blinding_accumulator + GroupAffine::<C>::new(x_i_lookup, y_i_lookup, false);

                (Some(index), x_left_minus_x_right_inv, delta, Some(blinding_accumulator.x), Some(blinding_accumulator.y))
            }
        };
    ...
    let (delta, x_l_minus_x_r_inv) = match (commitment) {
        Some(commitment) => {
            ...
            (Some(delta), Some(F::one()/(x_left-x_right)))// todo
