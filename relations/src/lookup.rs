...
fn b2f<F: Field>(v: bool) -> F {
    if v { F::one() } else { F::zero() }
}

fn is_bit<C: AffineCurve, Cs: ConstraintSystem<C>>(cs: &mut Cs, var: LinearCombination<C::ScalarField>) {
    let (_, _, zero) = cs.multiply(var.clone(), var - C::ScalarField::one());
    cs.constrain(zero.into());
}

fn bit<C: AffineCurve, Cs: ConstraintSystem<C>>(cs: &mut Cs, val: Option<bool>) -> Result<Variable<C::ScalarField>, R1CSError> {
    // compute assignment (if witness is provided)
    let assignment = val.map(|b| {
        let s = if b { C::ScalarField::one() } else { C::ScalarField::zero() };
...
fn single_membership<C: AffineCurve, Cs: ConstraintSystem<C>>(
    ...
    // left side
    let (_, _, left): (Variable<C::ScalarField>, Variable<C::ScalarField>, Variable<C::ScalarField>) = cs.multiply(s0.clone(), {
        let f = -(sa.clone() * u[0]) + (s2.clone() * u[0]) + (s1.clone() * u[0]) - u[0] + (sa.clone() * u[2]);
        let f = f - (s1.clone() * u[2]) + (sa.clone() * u[4]) - (s2.clone() * u[4]) - (sa.clone() * u[6]);
        let f = f + (sa.clone() * u[1]) - (s2.clone() * u[1]) - (s1.clone() * u[1]) + u[1] - (sa.clone() * u[3]);
        let f = f + (s1.clone() * u[3]) - (sa.clone() * u[5]) + (s2.clone() * u[5]) + (sa.clone() * u[7]);
        f
    });

    // right size
    let right = - (sa.clone() * u[0]) + (s2.clone() * u[0]) + (s1.clone() * u[0]) - u[0] + (sa.clone() * u[2]);
    let right = right - (s1.clone() * u[2]) + (sa.clone() * u[4]) - (s2.clone() * u[4]) - (sa.clone() * u[6]);
