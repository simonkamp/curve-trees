...
impl<C: AffineCurve> R1CSProof<C> {
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<R1CSProof<C>, R1CSError> {
        ...
        // let T_5 = C(read32!());
        // let T_6 = C(read32!());
        // let t_x = C::ScalarField::from_canonical_bytes(read32!()).ok_or(R1CSError::FormatError)?;
        // let t_x_blinding = C::ScalarField::from_canonical_bytes(read32!()).ok_or(R1CSError::FormatError)?;
        // let e_blinding = C::ScalarField::from_canonical_bytes(read32!()).ok_or(R1CSError::FormatError)?;
